...
import { 
  ...
  ChevronLeft, ChevronRight, Search, SearchCode, RotateCcw, LayoutGrid, Sun, Moon
} from 'lucide-react';
...
const LoginScreen = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    console.log('LoginScreen: Dark mode status applied to HTML:', darkMode);
  }, [darkMode]);
  ...
  if (publicStudent) {
    ...
    return ( // Public Student View
      <div className={`min-h-screen ${darkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-100 text-gray-800'} flex flex-col items-center p-0 md:p-6 overflow-y-auto transition-all duration-500`}>
        ...
        <div id="share-report-card" className="bg-white dark:bg-slate-900 w-full max-w-[800px] shadow-2xl relative my-auto print:shadow-none animate-in fade-in slide-in-from-bottom-4 duration-700 transition-colors">
            {/* Header Laporan */}
            <div className="bg-[#f2fdf5] dark:bg-slate-800 p-6 sm:p-8 border-b border-green-100 dark:border-slate-700 flex justify-between items-center dark:text-gray-100 transition-colors">
               <div>
                  <h1 className="text-2xl sm:text-3xl font-black text-[#111827] dark:text-white mb-1">Lesson Plan Al-Qur'an</h1>
                  <p className="text-[#00e676] font-bold text-sm italic dark:text-emerald-400">SDIT Al-Fityan School Bogor</p>
               </div>
               <div className="w-24 h-24 sm:w-32 sm:h-32 flex items-center justify-center shrink-0">
                  {institutionLogo ? <img src={institutionLogo} className="w-full h-full object-contain dark:invert transition-all" /> : <BookOpen size={48} className="text-green-600 dark:text-emerald-400" />}
               </div>
            </div>

            {/* Info Siswa */}
            <div className="p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-center gap-5 border-b border-gray-50 dark:border-slate-800 transition-colors">
               ...
               <div className="w-20 h-20 rounded-full bg-[#e6fbf0] border-4 border-[#00e676] text-[#00e676] flex items-center justify-center text-3xl font-black shrink-0 overflow-hidden">
                  {publicStudent.photo ? <img src={publicStudent.photo} className="w-full h-full object-cover" /> : <span className="dark:text-green-400 dark:bg-gray-700">{getInitials(publicStudent.name)}</span>}
               </div>
               <div>
                  <h2 className="text-2xl sm:text-3xl font-black text-gray-800 mb-2 dark:text-gray-100">{publicStudent.name}</h2>
                  <div className="flex flex-wrap gap-2">
                     <span className="bg-[#e6fbf0] text-green-800 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest dark:bg-green-900/50 dark:text-green-300">Kelas {publicStudent.kelas || '-'}</span>
                     <span className="bg-[#e6fbf0] text-green-800 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest dark:bg-green-900/50 dark:text-green-300">Kelompok {publicStudent.halaqoh || '-'}</span>
                  </div>
               </div>
               </div>
               ...
               <div className="hidden sm:flex flex-col items-center gap-1 shrink-0 print:flex transition-opacity">
                  <div className="w-16 h-16 bg-white p-1 border border-gray-100 rounded-lg shadow-sm dark:invert transition-all">
                     <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(`${window.location.origin}${window.location.pathname}?share=${publicStudent?.id}`)}`} alt="QR Code" className="w-full h-full dark:invert" crossOrigin="anonymous" />
                  </div>
            ...
            <div className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 px-6 py-4 flex justify-between items-center print:hidden transition-colors">
               <button onClick={() => changePublicWeek(-7)} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400 hover:text-emerald-500 dark:hover:bg-gray-700 dark:text-gray-300 dark:hover:text-emerald-400" title="Pekan Sebelumnya"><ChevronLeft size={24} className="dark:text-gray-400 dark:hover:text-emerald-400" /></button>
               ...
               <button onClick={() => changePublicWeek(7)} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400 hover:text-emerald-500 dark:hover:bg-gray-700 dark:text-gray-300 dark:hover:text-emerald-400" title="Pekan Selanjutnya"><ChevronRight size={24} className="dark:text-gray-400 dark:hover:text-emerald-400" /></button>
            </div>
            ...
            <div className="p-6 sm:p-8 flex flex-col gap-5 bg-gray-50/50 dark:bg-gray-900/50">
               {datesToDisplay.map((dateObj) => {
                  ...
                  return (
                    <div key={dateStr} className="bg-white border border-gray-100 rounded-[24px] p-5 shadow-sm print:break-inside-avoid dark:bg-gray-800 dark:border-gray-700">
                       <div className="flex justify-between items-center mb-4 border-b border-gray-50 pb-3 dark:border-gray-600">
                          ...
                          <span className="text-gray-400 font-bold italic text-sm">{formatShortDate(dateObj)}</span>
                       </div>
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
                          <div>
                             ...
                             <div className="text-sm font-bold text-gray-800 whitespace-pre-wrap dark:text-gray-200">{formatPrintData(rec[k.t], rec[k.h], rec[k.tNilai], rec[k.tsNilai])}</div>
                          </div>
                          <div>
                             ...
                             <div className="text-sm font-bold text-gray-800 whitespace-pre-wrap dark:text-gray-200">{formatPrintData(rec[k.f], rec[k.af], null, rec[k.fNilai])}</div>
                          </div>
                          <div>
                             ...
                             <div className="text-sm font-bold text-gray-800 whitespace-pre-wrap dark:text-gray-200">{formatPrintData(rec[k.m], '-', null, null)}</div>
                          </div>
                          <div>
                             ...
                             <div className="text-sm font-bold text-gray-800 whitespace-pre-wrap dark:text-gray-200">{rec[k.c] || '-'}</div>
                          </div>
                       </div>
                    </div>
                  );
            ...
            {/* Footer Laporan */}
            <div className="bg-[#111827] p-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-white dark:bg-gray-950 dark:text-gray-100">
        ...
        <div className="mt-8 mb-12 text-center flex flex-col items-center gap-2 dark:text-gray-400 dark:text-gray-500">
  ...
  if (isParentPortal) {
    ...
    return (
      <div className={`min-h-screen ${darkMode ? 'dark bg-slate-950' : 'bg-slate-50'} flex flex-col items-center relative overflow-hidden transition-colors duration-300`}>
         {/* HEADER PORTAL - AKSES LOGIN & DAFTAR */}
         <header className="w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 px-4 sm:px-8 py-4 flex justify-between items-center z-50 sticky top-0 shadow-sm transition-colors">
            <div className="flex items-center gap-2">
               <button onClick={() => setDarkMode(!darkMode)} className="p-2 text-gray-600 bg-gray-50 rounded-lg dark:bg-gray-700 dark:text-gray-300">
               ...
               <span className="font-black text-slate-800 tracking-tighter text-sm sm:text-base">MyQuranPlan</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
               <button 
                 onClick={() => { setIsParentPortal(false); setIsRegistering(false); setIsForgotPassword(false); }}
                 className="text-[10px] sm:text-xs font-bold text-slate-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 px-3 py-2 rounded-xl transition-colors"
               >
                 Masuk
               </button>
               <button 
         ...
         </header> {/* Removed redundant dark mode classes from header tag */}

         <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.015] pointer-events-none transition-opacity" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='80' height='80' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='${darkMode ? '%23ffffff' : '%2310b981'}'%3E%3Cpath d='M50 5 L62 38 L95 50 L62 62 L50 95 L38 62 L5 50 L38 38 Z' /%3E%3Cpath d='M50 5 L62 38 L95 50 L62 62 L50 95 L38 62 L5 50 L38 38 Z' transform='rotate(45 50 50)' /%3E%3C/g%3E%3C/svg%3E")`, backgroundSize: '100px 100px' }}></div>
         ...
         <div className="w-full max-w-4xl z-10 px-4 sm:px-8 py-8 md:py-12 flex-1">
            <div className="flex flex-col items-center mb-8 text-center px-4">
               <div className="w-24 h-24 mb-6 transition-transform hover:scale-110 duration-500">
                  {institutionLogo ? <img src={institutionLogo} className="w-full h-full object-contain dark:invert" /> : <BookOpen size={64} className="text-emerald-500 dark:text-emerald-400" />}
               </div>
               <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 animate-in fade-in slide-in-from-top-4 duration-1000">
               ...
               <h1 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tighter mb-2 dark:text-gray-100 dark:drop-shadow-[0_0_15px_rgba(52,211,153,0.4)] transition-all">MyQuranPlan</h1>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-white dark:border-slate-800 p-6 sm:p-8 transition-colors">
               <div className="relative mb-4">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl pl-12 pr-4 py-4 outline-none focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/10 text-lg font-bold text-slate-700 dark:text-slate-100 transition-all"
                  />
               </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                  <div className="relative group">
                    ...
                    <select 
                      ...
                      className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl pl-10 pr-3 py-3 text-xs font-bold text-slate-600 outline-none focus:border-emerald-500 focus:bg-white transition-all cursor-pointer appearance-none dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:focus:border-emerald-400 dark:focus:bg-gray-600"
                    >
               ...
               {isSearching && (
                 <button 
                   ...
                   className="mb-6 w-full py-3.5 bg-red-50 text-red-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-red-100 flex items-center justify-center gap-2 animate-in fade-in slide-in-from-top-1 duration-300 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-800/50"
                 >
                   <RotateCcw size={14} /> Reset Pencarian
                 </button>
               )}

               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[550px] overflow-y-auto custom-scrollbar pr-2">
                  {isPublicLoading ? (
                    <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-3">
                      {[...Array(6)].map((_, i) => (
                        <div key={i} className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl animate-pulse">
                          ...
                          <div className="w-5 h-5 bg-slate-100 rounded-md opacity-30 dark:bg-gray-700"></div>
                        </div>
                      ))}
                    </div>
                  ) : !isSearching ? (
                    ...
                    filtered.map((s, index) => (
                      <button 
                        ...
                        className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-700 rounded-2xl transition-all group animate-row-slide-in"
                        style={{ animationDelay: `${index * 0.05}s` }}
                      >
                        <div className="flex items-center gap-4 text-left">
                           <div className="w-12 h-12 rounded-full bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-500 font-black group-hover:border-emerald-500 group-hover:text-emerald-500 transition-colors overflow-hidden">
                              {s.photo ? <img src={s.photo} className="w-full h-full object-cover" /> : getInitials(s.name)}
                           </div>
                           <div>
                              <p className="font-black text-slate-800 dark:text-slate-100 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">{s.name}</p>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest dark:text-gray-300">Kelas {s.kelas || '-'} • {s.halaqoh || '-'}</p>
                           </div>
                        </div>
                        <ChevronRight size={20} className="text-slate-300 group-hover:text-emerald-500 transition-colors dark:text-gray-500 dark:group-hover:text-emerald-400" />
                      </button>
                    ))
                  ) : (
         ...
         <footer className="w-full py-10 text-center z-10 dark:bg-gray-900">
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest dark:text-gray-500">&copy; 2026 Juman Jayyidin. All rights reserved.</p>
         </footer>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-slate-950' : 'bg-slate-50'} flex flex-col justify-center items-center p-4 sm:p-6 relative overflow-hidden z-0`}>
      {/* Pola Islami Samar (Islamic Geometric Pattern) */}
      <div 
        className="absolute inset-0 opacity-[0.03] pointer-events-none -z-20"
        style={{ 
          backgroundImage: `url("data:image/svg+xml,%3Csvg width='80' height='80' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='${darkMode ? '%23374151' : '%2310b981'}'%3E%3Cpath d='M50 5 L62 38 L95 50 L62 62 L50 95 L38 62 L5 50 L38 38 Z' /%3E%3Cpath d='M50 5 L62 38 L95 50 L62 62 L50 95 L38 62 L5 50 L38 38 Z' transform='rotate(45 50 50)' /%3E%3C/g%3E%3C/svg%3E")`,
          backgroundSize: '100px 100px'
        }}
      ></div>

      {/* Efek Background Modern (Blobs) */}
      <div className="absolute top-[-10%] left-[-10%] w-[300px] h-[300px] sm:w-[500px] sm:h-[500px] bg-green-300/30 rounded-full mix-blend-multiply filter blur-[60px] sm:blur-[80px] opacity-70 animate-pulse -z-10 dark:bg-green-700/30"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[300px] h-[300px] sm:w-[500px] sm:h-[500px] bg-blue-300/30 rounded-full mix-blend-multiply filter blur-[60px] sm:blur-[80px] opacity-70 animate-pulse -z-10 dark:bg-blue-700/30" style={{ animationDelay: '2s' }}></div>

      <div className="w-full max-w-[420px] relative z-10 flex flex-col dark:text-gray-100">
        {/* Card Login / Register */}
        <div className="bg-white/80 backdrop-blur-2xl p-8 sm:p-10 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex flex-col relative overflow-hidden transition-all duration-500 dark:bg-gray-800/80 dark:border-gray-700 dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          ...
          <div className="flex flex-col items-center mb-10 mt-2">
             <div className="w-28 h-28 flex items-center justify-center mb-2">
                {institutionLogo && institutionLogo !== 'logo.png' ? (
                  ...
                  <div className="text-emerald-500/80 animate-in fade-in duration-500 dark:text-emerald-400/80">
                    {isRegistering ? <UserPlus size={64} /> : isForgotPassword ? <HelpCircle size={64} /> : <BookOpen size={64} />}
                  </div>
                )}
             </div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tighter dark:text-gray-100 dark:drop-shadow-[0_0_12px_rgba(52,211,153,0.4)] transition-all">MyQuranPlan</h1>
            <p className="text-sm text-slate-500 font-medium mt-2 text-center dark:text-gray-300">
          ...
          <form onSubmit={isForgotPassword ? handleResetRequest : handleAuth} className="flex flex-col gap-4 w-full">
            {isRegistering && (
              <div className="group animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 pl-1 transition-colors group-focus-within:text-emerald-500 dark:text-gray-400 dark:group-focus-within:text-emerald-400">Nama Lengkap & Gelar</label>
                <div className="relative flex items-center">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <GraduationCap size={18} className="text-slate-400 group-focus-within:text-emerald-500 transition-colors dark:text-gray-500 dark:group-focus-within:text-emerald-400" />
                  </div>
                  <input 
                    type="text" 
                    value={fullName} 
                    onChange={(e) => {setFullName(e.target.value); setError(''); setSuccessMsg('');}} 
                    className="w-full border-2 border-slate-100 rounded-2xl pl-11 pr-4 py-3.5 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 bg-slate-50/50 focus:bg-white text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300 placeholder:font-semibold dark:bg-gray-700/50 dark:border-gray-600 dark:focus:border-emerald-400 dark:focus:ring-emerald-400/20 dark:focus:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-400" 
            ...
            <div className="group">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 pl-1 transition-colors group-focus-within:text-emerald-500 dark:text-gray-400 dark:group-focus-within:text-emerald-400">Username</label>
              <div className="relative flex items-center">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <User size={18} className="text-slate-400 group-focus-within:text-emerald-500 transition-colors dark:text-gray-500 dark:group-focus-within:text-emerald-400" />
                </div>
                <input 
                  type="text" 
                  value={username} 
            ...
            {!isRegistering && !isForgotPassword && (
              <div className="text-right -mt-2">
                <button 
                  ...
                  className="text-[10px] font-bold text-emerald-600 hover:text-emerald-700 hover:underline transition-colors dark:text-emerald-400 dark:hover:text-emerald-300"
                >
                  Lupa Password?
                </button>
              </div>
            ...
            <div className="group">
              {!isForgotPassword && (
                <> {/* Added dark mode styles to password input */}
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 pl-1 transition-colors group-focus-within:text-emerald-500 dark:text-gray-400 dark:group-focus-within:text-emerald-400">Password</label>
                  <div className="relative flex items-center">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Lock size={18} className="text-slate-400 group-focus-within:text-emerald-500 transition-colors dark:text-gray-500 dark:group-focus-within:text-emerald-400" />
                    </div>
                    <input 
                      type={showPassword ? "text" : "password"} 
                      value={password} 
                      onChange={(e) => {setPassword(e.target.value); setError(''); setSuccessMsg('');}} 
                      className="w-full border-2 border-slate-100 rounded-2xl pl-11 pr-12 py-3.5 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 bg-slate-50/50 focus:bg-white text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300 placeholder:font-semibold dark:bg-gray-700/50 dark:border-gray-600 dark:focus:border-emerald-400 dark:focus:ring-emerald-400/20 dark:focus:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-400" 
                      placeholder="••••••••" 
                      required 
                    />
                    <button 
                      ...
                      className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 transition-colors dark:text-gray-500 dark:hover:text-gray-300"
            ...
            {isRegistering && (
              <div className="group animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 pl-1 transition-colors group-focus-within:text-emerald-500 dark:text-gray-400 dark:group-focus-within:text-emerald-400">Konfirmasi Password</label>
                <div className="relative flex items-center">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <Lock size={18} className="text-slate-400 group-focus-within:text-emerald-500 transition-colors dark:text-gray-500 dark:group-focus-within:text-emerald-400" />
                  </div>
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="mt-2 w-full bg-gradient-to-r from-[#00e676] to-emerald-500 hover:from-emerald-500 hover:to-[#00e676] text-white font-black py-4 rounded-2xl shadow-lg shadow-emerald-200 transition-all hover:shadow-xl hover:-translate-y-0.5 active:scale-[0.98] flex items-center justify-center gap-2 text-sm disabled:opacity-70 disabled:cursor-not-allowed dark:from-emerald-600 dark:to-emerald-500 dark:hover:from-emerald-500 dark:hover:to-emerald-600 dark:shadow-emerald-900/50"
            >
          ...
          <div className="mt-6 flex flex-col gap-3 text-center border-t border-slate-100 pt-5">
             {!isRegistering && !isForgotPassword && (
               <button 
                ...
                className="w-full bg-slate-50 text-slate-500 font-black py-4 rounded-2xl hover:bg-slate-100 transition-all flex items-center justify-center gap-2 text-sm dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
               >
                  <Search size={18} /> Kembali ke MyQuranPlan
               </button>
             )}
             <button onClick={() => { if(isForgotPassword) setIsForgotPassword(false); else setIsRegistering(!isRegistering); setError(''); setSuccessMsg(''); setConfirmPassword(''); }} className="text-xs text-emerald-600 font-bold hover:text-emerald-700 hover:underline transition-colors dark:text-emerald-400 dark:hover:text-emerald-300">
        ...
        <p className="text-xs text-slate-400 font-bold mt-8 text-center drop-shadow-sm dark:text-gray-500">&copy; 2026 Juman Jayyidin. All rights reserved.</p>
      </div>
      ...
      {error && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-[200] p-4 transition-opacity animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[320px] overflow-hidden flex flex-col items-center p-8 text-center animate-in zoom-in-95 duration-300 dark:bg-gray-800">
            ...
            <h3 className="text-xl font-black text-slate-800 mb-2 dark:text-gray-100">{isRegistering ? 'Gagal Mendaftar' : 'Akses Ditolak'}</h3>
            <p className="text-sm text-slate-500 font-medium mb-8 leading-relaxed dark:text-gray-300">{error}</p>
            <button onClick={() => setError('')} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-black py-3.5 rounded-xl transition-colors text-sm active:scale-95 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100">Mengerti</button>
          </div>
        </div>
      )}
      ...
      {successMsg && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-[200] p-4 transition-opacity animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[320px] overflow-hidden flex flex-col items-center p-8 text-center animate-in zoom-in-95 duration-300 dark:bg-gray-800">
            ...
            <h3 className="text-xl font-black text-slate-800 mb-2 dark:text-gray-100">Pendaftaran Berhasil!</h3>
            <p className="text-sm text-slate-500 font-medium mb-8 leading-relaxed dark:text-gray-300">{successMsg}</p>
