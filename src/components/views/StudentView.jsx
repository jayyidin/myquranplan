...
import { Users, Settings, Plus, Edit3, Trash2 } from 'lucide-react';

const StudentView = ({ activeHalaqoh, filteredStudents, openAddStudentModal, openEditStudentModal, requestDeleteStudent, isSuperAdmin }) => {
          ...
          {filteredStudents.map(student => (
            <div key={student.id} className="bg-white p-4 md:p-6 min-h-[110px] md:min-h-[130px] rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 flex items-start md:items-center justify-between hover:shadow-md transition-all group gap-3">
              <div className="flex items-center gap-3 md:gap-4 min-w-0 flex-1">
                <div className={`w-[48px] h-[48px] md:w-[60px] md:h-[60px] rounded-full flex items-center justify-center font-black text-lg md:text-xl overflow-hidden relative shrink-0 border border-gray-100 ${student.color || 'bg-blue-100 text-blue-600'}`}>
