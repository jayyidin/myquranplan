        ...
        {addStudentMode === 'master' ? (
          <div className="p-4 flex flex-col gap-3 h-[380px]">
            <div className="relative shrink-0">
              ...
              <input autoFocus type="text" placeholder="Cari nama siswa..." value={localQuery} onChange={e => setLocalQuery(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-9 pr-4 py-2.5 text-sm font-bold outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2">
              {isSearching ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex items-center justify-between bg-white border border-gray-100 p-2.5 rounded-xl shadow-sm animate-pulse">
                    ...
                    <div className="w-8 h-8 rounded-lg bg-slate-100 shrink-0 ml-3"></div>
                  </div>
                ))
              ) : students.filter(s => s.name.toLowerCase().includes(masterSearchQuery.toLowerCase())).length === 0 ? (
                <div className="text-center text-xs text-gray-400 py-8 font-bold flex flex-col items-center gap-2"><User size={24} className="text-gray-300" /> {students.length === 0 ? 'Tidak ada data siswa yang halaqohnya kosong.' : 'Siswa tidak ditemukan.'}</div>
